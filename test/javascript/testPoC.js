...
const deployValorToken = () => {
    return ValorTokenMockup.at(ValorTokenMockup.address)
}
...
const deployValorStakeFactory = () => {
    return ValorStakeFactory.at(ValorStakeFactory.address)
}
...
contract('ValorTimelock', (accounts) => {
  ...
  it('allows doing emergency release only from multisig transaction', async () => {
    ...
    // will transfer funds to the multisig wallet
    await valorTokenMockup.transfer(multisigInstance.address, funds, {from: accounts[0]});
    assert.equal(await valorTokenMockup.balanceOf(multisigInstance.address), funds)
